...
class LpipsMetric:
    ...
    def __init__(self, mesh: o3d.geometry.TriangleMesh, cam_params: dict):
        self.mesh = mesh
        self.intrinsic, self.extrinsic, self.width, self.height = _parse_cam_params(
            cam_params
        )
        self.prepare_mesh(self.mesh)
        self.gt_image = self.extract_image_from_mesh(self.mesh)
    ...
    def from_mesh_dir(cls, base_dir: Path):
        ...
        cam_params_path = base_dir / "camera_params.json"

        if not mesh_path.exists():
            raise FileNotFoundError(f"Mesh file not found: {mesh_path}")
        if not cam_params_path.exists():
            raise FileNotFoundError(
                f"Camera parameters file not found: {cam_params_path}"
            )
    ...
    def extract_image_from_mesh(
        self, mesh: o3d.geometry.TriangleMesh, output_path: Path | None = None
    ) -> torch.Tensor:

        # Create offscreen renderer
        render = rendering.OffscreenRenderer(self.width, self.height)
        ...
        camera_position = self.extrinsic[:3, 3]
        forward = -self.extrinsic[:3, 2]
        up = self.extrinsic[:3, 1]
        ...
        fov_rad = 2 * np.arctan(1.0 / self.intrinsic[1, 1])
    ...
    def __call__(
        ...
        pred_image = self.extract_image_from_mesh(
            other_mesh,
            save_path.with_name(f"{save_path.stem}_pred.png"),
        )
        lpips_score = self.lpips(pred_image, self.gt_image)
        return {"metrics_main/lpips": float(lpips_score.item())}
