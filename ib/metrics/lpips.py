...
def load_json(cam_params_path: Path) -> dict:
    with open(cam_params_path, "r") as f:
        cam_params = json.load(f)
    return cam_params


def parse_cam_params(cam_params: dict) -> tuple[np.ndarray, np.ndarray, int, int]:
...
class LpipsMetric:
    ...
    def from_mesh_dir(cls, base_dir: Path):
        ...
        for cam_file in cam_param_files:
            ...
            if match:
                mode = match.group(1)
                cam_params_dict = load_json(cam_file)
    ...
    def __call__(
        ...
        for mode, cam_params in self.all_cam_params:
            ...
            gt_image = extract_image_from_mesh(
                self.mesh,
                width,
                height,
                extrinsic,
            ...
            pred_image = extract_image_from_mesh(
                other_mesh,
                width,
                height,
                extrinsic,
